...
if ( ! class_exists( 'Scuba_Diving_Sport_Plugin_Activation_WPElemento_Importer' ) ) {
    ...
    class Scuba_Diving_Sport_Plugin_Activation_WPElemento_Importer {
        ...
        public function __construct() {
            ...
            $actions                   = $this->scuba_diving_sport_get_recommended_actions();
        ...
        public function scuba_diving_sport_recommended_elemento_importer_plugins_array($plugins){
            $plugins[] = array(
        ...
        public function scuba_diving_sport_get_recommended_actions() {
            ...
            $plugins = $this->scuba_diving_sport_get_recommended_plugins();
            ...
            if ($plugins) {
                foreach ($plugins as $key => $plugin) {
                    ...
                    $link_and_is_done  = $this->scuba_diving_sport_get_plugin_buttion($plugin['slug'], $plugin['name'], $plugin['function']);
        ...
        public function scuba_diving_sport_get_recommended_plugins() {
            ...
            $plugins = apply_filters('scuba_diving_sport_recommended_plugins', array());
            return $plugins;
        }
        ...
        public function scuba_diving_sport_get_plugin_buttion($slug, $name, $function) {
                ...
                if (!$is_installed) {
                    ...
                    $button_html        = sprintf('<a class="scuba-diving-sport-plugin-install install-now button-secondary button" data-slug="%1$s" href="%2$s" aria-label="%3$s" data-name="%4$s">%5$s</a>',
                        esc_attr($slug),
                    ...
                    $button_html = sprintf('<a class="scuba-diving-sport-plugin-activate activate-now button-primary button" data-slug="%1$s" href="%2$s" aria-label="%3$s" data-name="%4$s">%5$s</a>',
                        esc_attr($slug),
                    ...
                    $button_html = sprintf('<div class="action-link button disabled"><span class="dashicons dashicons-yes"></span> %s</div>', esc_html__('Active', 'scuba-diving-sport'));
                    $is_done     = true;
                }
